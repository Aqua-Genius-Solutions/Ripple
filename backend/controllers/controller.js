...
async function likeEvent(req, res) {
  try {
    ...
    const isLiked = user.LikedEvents.some((likedEvent) => likedEvent.id === eventId);
    if (isLiked) {
...
async function participateInEvent(req, res) {
  try {
    ...
    const isParticipated = user.ParticipatedEvents.some((participatedEvent) => participatedEvent.id === eventId);
    if (isParticipated) {
      return res.status(400).json({ message: 'User has already participated in the event' });
    }
    ...
    return res.status(200).json({ message: 'User participated in the event successfully' });
  } catch (error) {
